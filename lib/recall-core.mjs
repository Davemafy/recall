...
const CITE_RE = /(?:(?<case>[A-Z][A-Za-z0-9.'&()\- ]{1,80}\s+v\.\s+[A-Z][A-Za-z0-9.'&()\- ]{1,80}),?\s*)?(?<volume>\d{1,4})\s+(?<reporter>U\.\s*S\.|U\.S\.|F\.\s*(?:2d|3d|4th)?|S\.\s*Ct\.|Fed\.\s*Appx\.)\s+(?<page>\d{1,5})(?:\s*,\s*(?:at\s+)?(?<pin>\d{1,5}))?(?:\s*\((?<courtYear>[^)]{2,60})\))?/gi;
  ...
  for(const doc of documents) {
    ...
    if(!criticalIncident && incidentAuthority.quote) {
      quotes.forEach((q,i)=>{
        ...
        if(score>=0.82) edges.push({id:`quote:${doc.id}:${i}`,documentId:doc.id,type:'CONFIRMED_QUOTE_REUSE',confidence:'DETERMINISTIC',evidence:{raw:q.raw,score,rule:'normalized quote overlap >= 0.82'}});
      });
    }
