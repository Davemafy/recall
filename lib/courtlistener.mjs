...
export function classifyPublicCandidateRelationships(candidate,incident){
  ...
  if(incident?.exactTextDependency&&exactPhraseOccurrence(text,incident.exactTextDependency)){
    relationships.push({classification:RELATIONSHIP_STATE.CONFIRMED_CITATION,evidence:{raw:incident.exactTextDependency,rule:'normalized exact incident-dependency occurrence',sourceText:text,confirmationSource}});
  }
...
export async function traceCourtListener({input,quote='',exactTextDependency='',maxCandidates=25,broad=false,signal},token,{firecrawlKey=process.env.FIRECRAWL_API_KEY||'',diagnostics=createTraceDiagnostics()}={}){
  ...
  const authority=resolved.authority;const rawInput=stripHtml(input||'').trim();const effectiveQuote=quote||(!authority&&!exactTextDependency&&rawInput.length>=28?rawInput:'');
  const incident={...authority,quote:effectiveQuote,proposition:effectiveQuote,exactTextDependency:stripHtml(exactTextDependency||'').trim(),canonicalId:authority?.canonicalId||null};
  ...
  const cacheKey=createHash('sha256').update(JSON.stringify({primary:primary?.q,quote:effectiveQuote,exactTextDependency:incident.exactTextDependency,maxCandidates,broad})).digest('hex');
