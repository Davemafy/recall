...
const escapePhraseToken=(value='')=>String(value).replace(/[|\\{}()[\]^$+*?.]/g,'\\export function quoteSimilarity(a,b) {
  const A=normalizeQuote(a), B=normalizeQuote(b);
  if(A===B) return 1;
  if(A.length>30 && B.length>30 && (A.includes(B)||B.includes(A))) return Math.min(A.length,B.length)/Math.max(A.length,B.length);
  return jaccard(A,B);
...
export function findVerbatimPhraseSpan(text='',phrase='') {
  const normalized=normalizeQuote(phrase);
  if(normalized.length<24) return null;
  const tokens=normalized.split(/\s+/).filter(Boolean);
  if(tokens.length<3) return null;
  const pattern=tokens.map(escapePhraseToken).join('[\\s\\p{P}\\p{S}]+');
  try{
    const match=new RegExp(pattern,'iu').exec(String(text));
    if(!match) return null;
    return {raw:match[0],start:match.index,end:match.index+match[0].length,score:1,rule:'verbatim normalized phrase occurrence'};
  }catch{return null}
}
...
export function classifySnippet(snippet, incident={canonicalId:canonicalId(999,'F.4th',123), quote:INCIDENT_QUOTE, proposition:INCIDENT_PROPOSITION}) {
  ...
  if(direct) return {type:'CONFIRMED_CITATION_DEPENDENCY',evidence:{raw:direct.raw,canonical:direct.canonicalId,rule:'reporter-volume-first_page match'}};
  const q=extractQuotes(snippet).map(q=>({q,score:quoteSimilarity(q.raw,incident.quote)})).sort((a,b)=>b.score-a.score)[0];
  if(q && q.score>=0.82 && !/(fabricat|invalid|do not rely|cannot be verified)/i.test(snippet)) return {type:'CONFIRMED_QUOTE_REUSE',evidence:{raw:q.q.raw,score:q.score,rule:'normalized quote overlap >= 0.82'}};
  const verbatim=findVerbatimPhraseSpan(snippet,incident.quote);
  if(verbatim && !/(fabricat|invalid|do not rely|cannot be verified)/i.test(snippet)) return {type:'CONFIRMED_QUOTE_REUSE',evidence:verbatim};
  const props=extractPropositions(snippet).map(text=>({text,score:jaccard(text,incident.proposition)})).sort((a,b)=>b.score-a.score)[0];
  if(props && props.score>=0.33 && !/(fabricat|invalid|do not rely|cannot be verified)/i.test(snippet)) return {type:'POSSIBLE_DERIVED_CLAIM',evidence:{raw:props.text,score:props.score,rule:'lexical semantic review threshold'}};
  return {type:'NOT_RELATED',evidence:null};
}
...
export function analyzeCorpus(documents, incidentAuthority={citation:INCIDENT_CITATION,canonicalId:canonicalId(999,'F.4th',123),quote:INCIDENT_QUOTE,proposition:INCIDENT_PROPOSITION}) {
  ...
  for(const doc of documents) {
    ...
    direct.forEach((c,i)=>edges.push({id:`cite:${doc.id}:${i}`,documentId:doc.id,type:'CONFIRMED_CITATION_DEPENDENCY',confidence:'DETERMINISTIC',evidence:{raw:c.raw,canonical:c.canonicalId,rule:'reporter-volume-first_page match',surrounding:c.surrounding}}));
    ...
    if(!criticalIncident && incidentAuthority.quote) {
      ...
      quotes.forEach((q,i)=>{
        ...
        if(score>=0.82) {
          quoteMatched=true;
          edges.push({id:`quote:${doc.id}:${i}`,documentId:doc.id,type:'CONFIRMED_QUOTE_REUSE',confidence:'DETERMINISTIC',evidence:{raw:q.raw,score,rule:'normalized quote overlap >= 0.82'}});
        }
      });
      if(!quoteMatched){
        const verbatim=findVerbatimPhraseSpan(doc.text,incidentAuthority.quote);
        if(verbatim) edges.push({id:`quote:${doc.id}:verbatim`,documentId:doc.id,type:'CONFIRMED_QUOTE_REUSE',confidence:'DETERMINISTIC',evidence:verbatim});
      }
    }
    if(!direct.length&&!criticalIncident&&incidentAuthority.proposition) {
      ...
      if(best&&best.score>=0.33&&!otherAuthority) edges.push({id:`possible:${doc.id}:${best.i}`,documentId:doc.id,type:'POSSIBLE_DERIVED_CLAIM',confidence:'HEURISTIC_REVIEW',evidence:{raw:best.p,score:best.score,rule:'lexical semantic review threshold'}});
    }
  ...
  const confirmedDocs=documents.filter(d=>(byDoc.get(d.id)||[]).some(e=>e.type!=='POSSIBLE_DERIVED_CLAIM'));
  const possibleDocs=documents.filter(d=>(byDoc.get(d.id)||[]).some(e=>e.type==='POSSIBLE_DERIVED_CLAIM') && !confirmedDocs.some(c=>c.id===d.id));
  ...
  const summary={
    ...
    confirmedCitationDependencies:edges.filter(e=>e.type==='CONFIRMED_CITATION_DEPENDENCY').length,
    confirmedQuoteReuse:edges.filter(e=>e.type==='CONFIRMED_QUOTE_REUSE').length,
    possibleDerivedClaims:edges.filter(e=>e.type==='POSSIBLE_DERIVED_CLAIM').length,
...
export function remediationFor(documents, analysis) {
  ...
  for(const doc of analysis.affectedDocuments) {
    ...
    for(const edge of docEdges) {
      ...
      if(edge.type!=='POSSIBLE_DERIVED_CLAIM') {
        ...
        else {priority='HIGH';action=edge.type==='CONFIRMED_QUOTE_REUSE'?'Verify reused quotation against authoritative source':'Replace or remove invalid authority after review';}
      }
...
export function graphFor(documents, analysis, incidentAuthority={caseName:INCIDENT_CASE,citation:INCIDENT_CITATION}) {
  ...
  for(const doc of analysis.affectedDocuments) {
    ...
    docEdges.forEach((edge,idx)=>{
      if(edge.type==='CONFIRMED_CITATION_DEPENDENCY') edges.push({id:`g:${edge.id}`,source:'authority:incident',target:`doc:${doc.id}`,type:edge.type,evidence:edge.evidence});
      else {
        const mid=`${edge.type==='CONFIRMED_QUOTE_REUSE'?'quote':'prop'}:${doc.id}:${idx}`;
        nodes.push({id:mid,type:edge.type==='CONFIRMED_QUOTE_REUSE'?'QUOTE':'PROPOSITION',label:edge.type==='CONFIRMED_QUOTE_REUSE'?'Reused quotation':'Possible proposition',documentId:doc.id});
