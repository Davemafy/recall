...
function applyRecordedDeterministicEvidence(analysis){
  ...
  analysis.summary.possibleDerivedClaims=analysis.edges.filter(edge=>edge.type==='POSSIBLE_RELATED_PROPOSITION').length;
  return analysis;
}
...
export function getRecordedPublicTrace(){
  ...
  const confirmed=RECORDED_PUBLIC_FILINGS.filter(document=>(analysis.byDoc.get(document.id)||[]).some(edge=>edge.type!=='POSSIBLE_RELATED_PROPOSITION'));
