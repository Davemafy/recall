...
export function extractDocumentsFromSearchPayload(payload){
  const out=[]; const seen=new Set();
  const push=(raw,parent={})=>{
    if(!raw||typeof raw!=='object') return;
    const doc=normalizeSearchDocument(raw,parent);
...
export async function traceCourtListener({input,quote='',maxCandidates=50},token){
  ...
  for(const pass of passes){
    ...
    const docs=[...extractDocumentsFromSearchPayload(docketSearch.data),...extractDocumentsFromSearchPayload(docSearch.data)];
