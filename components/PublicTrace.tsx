...
type TraceResult={ok?:boolean;sourceState?:string;retrievedAt?:string;capturedAt?:string;authority?:any;authorityResolution?:string;documents:TraceDoc[];summary:any;coverage:any;sourceQueries?:any[];mode?:string};
...
export default function PublicTrace({recorded=false}:{recorded?:boolean}){
 ...
 const run=async()=>{
   ...
   try{
     const res=await fetch('/api/trace',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({input,quote,maxCandidates:50})});
 ...
 const label=recorded?'RECORDED PUBLIC TRACE':result?.sourceState==='CACHED'?'CACHED PUBLIC SOURCE':'LIVE PUBLIC SOURCE';
 return <main className="publicTracePage">
   ...
   {result&&<>
     <section className="traceSummary">
       ...
       <div className="coverageNote">{result.coverage?.bounded?<strong>Bounded search.</strong>:<strong>Checked returned result set.</strong>} <span>RECALL checked {summary?.checkedCount??docs.length} filing candidates. Search counts above 2,000 can be approximate at the source.</span></div>
     </section>
