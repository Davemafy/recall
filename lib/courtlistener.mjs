...
export function confirmPublicCandidate(candidate, incident){
  ...
  if(incident?.canonicalId){
    ...
    if(cites.length){
      const c=cites[0];
      return {classification:'CONFIRMED_CITATION_DEPENDENCY',evidence:{raw:c.raw,canonical:c.canonicalId,rule:'reporter-volume-first_page match',sourceText:c.surrounding||text,confirmationSource:candidate?.fullText?'RECAP_PLAIN_TEXT':'SEARCH_SNIPPET'}};
    }
    if(incidentCites.length) return {classification:'NOT_RELATED',reason:'The available context discusses the incident authority critically rather than relying on it.',evidence:{raw:incidentCites[0].raw,rule:'critical-discussion exclusion',sourceText:incidentCites[0].surrounding||text}};
    const short=incidentShortCitationOccurrences(text,incident);
    if(short.length) return {classification:'CONFIRMED_CITATION_DEPENDENCY',evidence:{raw:short[0].raw,canonical:incident.canonicalId,rule:short[0].rule,sourceText:text,confirmationSource:candidate?.fullText?'RECAP_PLAIN_TEXT':'SEARCH_SNIPPET'}};
  }
  if(incident?.quote){
    ...
    if(best.score>=0.82) return {classification:'CONFIRMED_QUOTE_REUSE',evidence:{raw:best.raw,score:best.score,rule:'normalized quote overlap >= 0.82',sourceText:text,confirmationSource:candidate?.fullText?'RECAP_PLAIN_TEXT':'SEARCH_SNIPPET'}};
  }
...
export async function traceCourtListener({input,quote='',maxCandidates=50},token){
  ...
  for(const pass of passes){
    ...
    for(const candidate of candidates){
      ...
      seen.add(key);
      const hydrated=await hydratePublicDocument(candidate,token);
      const normalized=normalizeTraceDocuments([hydrated],incident,pass.q,retrievedAt)[0];
      all.push(normalized);
    }
  }
